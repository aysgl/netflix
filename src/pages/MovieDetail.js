...
const MoviemovieDetail = () => {
    ...
    return (
        <div>
            {state?.movieDetail?.ismovieDetailLoading && state?.similar?.isSimilarLoading ? (
                <Loading />
            ) : state?.movieDetail?.ismovieDetailError && state?.similar?.isSimilarError ? (
                <p>Error loading movie movieDetails</p>
            ) : (
                <div className='container mt-5 pt-5'>
                    <div className='row g-0'>
                        <div className='col-md-5'>
                            <div className='play'>
                                <img className='img-fluid' src={`${baseImageUrl.concat(state?.movieDetail.movieDetails?.poster_path)}`} alt='' />

                                <Link target='_blank' to={state?.movieDetail?.movieDetails?.homepage} className='btn btn-danger'><svg fill='white' xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 384 512"><path d="M73 39c-14.8-9.1-33.4-9.4-48.5-.9S0 62.6 0 80V432c0 17.4 9.4 33.4 24.5 41.9s33.7 8.1 48.5-.9L361 297c14.3-8.7 23-24.2 23-41s-8.7-32.2-23-41L73 39z" /></svg></Link>
                            </div>
                        </div>
                        <div className='col-md-1'></div>
                        <div className='col-md-6'>
                            <p className='display-2 d-flex justify-content-between align-items-end'>
                                <span>{state?.movieDetail.movieDetails?.title}</span>
                                <span className='h2 d-flex align-items-center'>
                                    {state?.movieDetail.movieDetails?.vote_average !== undefined && (
                                        <>
                                            {state?.movieDetail.movieDetails?.vote_average.toFixed(1)}
                            ...
                            <p className='text-secondary'>
                                {state?.movieDetail?.movieDetails?.release_date?.slice(0, 4)} | {state?.movieDetail?.movieDetails.adult > true ? "12+" : "18+"}
                            </p>
                            <p className=''>{state?.movieDetail?.movieDetails?.overview}</p>
                            <div className='text-secondary'>{state?.movieDetail?.movieDetails?.genres?.map(i => <span className='me-2' key={i.id}>{i.name}</span>)}</div>


                            <p className='display-6 pt-5'>Similar movies</p>
...
export default MoviemovieDetail;
